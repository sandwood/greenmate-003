...
userSchema.statics.authenticate = function() {
  return function(id, password, done) {
    User.findOne({ $and: [{ id: id }, { block: 0 }] }).exec(function(
      error,
      user
    ) {
...
userSchema.statics.signup = function(req, res) {
  ...
  User.findOne({ $and: [{ id: id }, { block: 0 }] }).exec(function(
    ...
    if (user) {
      return res
        .status(400)
        .json({ isSuccess: 0, error: "that user exists already." });
    }
    if (!user) {
...
userSchema.statics.searchUser = function(req, res) {
  ...
  User.find({ $and: [{ id: { $regex: req.body.userId } }, { block: 0 }] })
    ...
    .exec(function(error, users) {
      ...
      }
      {
        ...
        user_elements.ids = userids;
        user_elements.usernames = usernames;
        User.find({
          $and: [{ id: { $regex: req.body.userId } }, { block: 0 }]
    ...
    if (err) res.status(err.code).json({ isSuccess: 0 });
      ...
      var sendPwdReset = transporter.templateSender(
        {
          ...
          html:
            '<b>Hello, {{username}}! This is Green Mate✔ </b></p><p>Please <a href="{{ reset }}"><b>go here </b></a>to reset your password :)</p>'
        },
        {
      ...
      sendPwdReset(
        ...
        },
        {
          username: user.username,
          reset:
            "https://green-mate2-petercha90.c9users.io/resetuserpassword?id=" +
            encodedId
        },
        function(err, info) {
          if (err) {
...
userSchema.statics.checkPassword = function(req, res) {
  ...
  User.findOne({ id: decodedId }).exec(function(error, user) {
    if (error) {
...
userSchema.statics.resetPassword = function(req, res) {
  ...
  User.update(
    { id: userid },
    { password: hash, accessToken: newToken },
    { multi: false },
    function(err, tasks) {
      if (err) {
        console.log(err);
  ...
  return res
    .status(200)
    .json({ isSuccess: 1, result: "Password changed successfully" });
};
