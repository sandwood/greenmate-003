...
var connection = mongoose.createConnection(
  "mongodb://krazylab:eoqkr2014@aws-us-west-2-portal.2.dblayer.com:15914/green_mate"
);
      ...
      Question.find({ block: 0 }).count(function(err, count) {
        ...
        questions.forEach(function(question) {
          ...
          question.comments.forEach(function(comment) {
            if (comment.writer == 1) numOfManagerReply += 1;
          });
...
questionSchema.statics.getUserQuestions = function(req, res) {
  ...
  Question.find({ $and: [{ userSeq: userSeq }, { block: 0 }] })
    .sort({ published_date: -1 })
    .skip((page - 1) * num)
    .limit(num)
    .exec(function(err, questions) {
      ...
      Question.find({ $and: [{ userSeq: userSeq }, { block: 0 }] }).count(
        function(err, count) {
          if (err) return res.status(err.code).json({ isSuccess: 0 });
          var questionList = [];
          questions.forEach(function(question) {
...
questionSchema.statics.getSolved = function(req, res) {
  ...
  Question.find({ $and: [{ solved: true }, { block: 0 }] })
    .sort({ published_date: -1 })
    .skip((page - 1) * num)
    .limit(num)
    .exec(function(err, questions) {
...
questionSchema.statics.getUserSolved = function(req, res) {
  ...
  Question.find({
    $and: [{ userSeq: userSeq }, { solved: true }, { block: 0 }]
  })
    .sort({ published_date: -1 })
    .skip((page - 1) * num)
    .limit(num)
    .exec(function(err, questions) {
      ...
      Question.find({ $and: [{ userSeq: userSeq }, { solved: true }] }).count(
        function(err, count) {
          if (err) return res.status(err.code).json({ isSuccess: 0 });
          if ((page - 1) * num + num >= count) {
            return res.status(200).json({
...
questionSchema.statics.getUnsolved = function(req, res) {
  ...
  Question.find({ $and: [{ solved: false }, { block: 0 }] })
    .sort({ published_date: -1 })
    .skip((page - 1) * num)
    .limit(num)
    .exec(function(err, questions) {
      ...
      Question.find({ $and: [{ solved: false }, { block: 0 }] }).count(function(
        err,
        count
      ) {
        if (err) return res.status(err.code).json({ isSuccess: 0 });
...
questionSchema.statics.getUserUnsolved = function(req, res) {
  ...
  Question.find({
    $and: [{ userSeq: userSeq }, { block: 0 }, { solved: false }]
  })
    .sort({ published_date: -1 })
    .skip((page - 1) * num)
    .limit(num)
    .exec(function(err, questions) {
      ...
      Question.find({ $and: [{ userSeq: userSeq }, { solved: false }] }).count(
        function(err, count) {
          if (err) return res.status(err.code).json({ isSuccess: 0 });
          if ((page - 1) * num + num >= count) {
            return res.status(200).json({
        ...
        Question.find({ block: 0 })
          ...
          .count(function(error, count) {
            ...
            if (keyword != null) {
              ...
              questions.forEach(function(question) {
                if (
                  question.title.toLowerCase().indexOf(keyword) > -1 ||
                  question.contents.toLowerCase().indexOf(keyword) > -1 ||
                  question.username.toLowerCase().indexOf(keyword) > -1
                ) {
                  var numOfManagerReply = 0;
