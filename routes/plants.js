...
var apiKey =
  "AAAAPTzFJJM:APA91bHMuqNIHv_IDhhsT08BoJvkWHaCzUPdXV-VzLbtPtk4v9dYJQ0jpcjYsE745dCr2pPrIA1anDx02ZTTzUAj0_2Gz588RIBb5SHbEU8QY3-3HjjhlEFu6ZReM-zF_n4Q91SEsF1WUIgdBTx2Ao_UD_NmZMRxtw";
...
router.post("/addPlant", function(req, res) {
  ...
  User.findOne({ $and: [{ id: userId }, { block: 0 }] }).exec(function(
    err,
    user
  ) {
    if (err) {
      ...
      User.findOne({ userSeq: req.headers["userseq"] }).exec(function(
        err,
        manager
      ) {
        if (err) {
        ...
        if (manager.username) managerName = manager.username;
        // if(manager.factory)
        ...
        var plant = new Plant({
          ...
          diaryCount: 0,
          block: 0,
          published_date: moment(Date.now())
            .tz("Asia/Tokyo")
        ...
        User.update({ id: userId }, { $set: { plants: plantList } }, function(
          err,
          tasks
        ) {
          if (err) {
...
router.post("/harvest", function(req, res) {
  ...
  Plant.findOne({ plantId: plantId }).exec(function(err, plant) {
    if (err) {
      ...
      Plant.update({ plantId: plantId }, { $set: { harvest: 1 } }, function(
        err,
        tasks
      ) {
        if (err) {
          ...
          User.findOne({ $and: [{ id: userId }, { block: 0 }] }, function(
            err,
            user
          ) {
            if (err) {
              ...
              deviceId.forEach(function(device) {
                ...
                if (device.os == "android") {
                  ...
                  var message = {
                    ...
                    notification: {
                      title: "It's Green Mate :)",
                      body:
                        "It's time to harvest your " + plant.plantName + "!",
                      sound: "default"
                    },
                    data: {
                      // title: "It's Green Mate :)",
                ...
                fcm.send(message, function(err, messageId) {
                  if (err) {
                    console.log(
                      "-> fcm send error : " + err + "message:" + message
                    );
                    // return res.status(err.code).json({isSuccess : 0});
                  } else {
                    ...
                    newNoti.save(function(err) {
                      if (err)
                        return res
                          .status(err.code)
                          .json({ isSuccess: 0, err: err });
                      console.log("Sent with message ID: ", messageId);
                      User.update(
                        { $and: [{ id: userId }, { block: 0 }] },
                        { $pull: { plants: { plantId } } }
                      );
                    });
                  }
...
router.post("/getUserPlants", function(req, res) {
  ...
  Plant.find({ $and: [{ userSeq: userSeq }, { block: 0 }] })
    .sort({ published_date: -1 })
    .skip((page - 1) * num)
    .limit(num)
    .exec(function(err, plants) {
      ...
      Plant.find({ userSeq: userSeq }).count(function(err, count) {
        ...
        plants.forEach(function(data) {
          if (data.picUrl) picUrl = data.picUrl;
