...
router.post("/getQuestionDetail", function(req, res) {
  Question.findOne({ questionId: req.body.questionId }).exec(function(
    err,
    question
  ) {
    if (err) return res.status(err.code).json({ isSuccess: 0, err: err });
...
router.post("/getComments", function(req, res) {
  ...
  Question.findOne({ questionId: questionId }, function(error, question) {
    if (error) return res.status(error.code).json({ isSuccess: 0, err: error });
      ...
      return res.status(204).json({ isSuccess: 0, error: "No content" });
...
router.post("/addComment", function(req, res) {
  ...
  Question.findOne(
    { $and: [{ questionId: questionId }, { block: 0 }] },
    function(error, question) {
      ...
      if (error)
        ...
        User.findOne({ userSeq: req.headers["userseq"] }).exec(function(
          ...
          if (comment.writer == 1) question.solved = true;
